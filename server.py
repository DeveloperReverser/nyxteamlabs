...
Что делает:
  1. /api/lead — принимает заявку с сайта (имя + телефон/почта/телеграм + задача)
     и пересылает её тебе в Telegram.
  2. /api/chat/* — простой живой чат на сайте.

Переменные окружения (обязательные):
  - BOT_TOKEN  — токен Telegram-бота
  - CHAT_ID    — твой Telegram ID (куда приходят уведомления)
  - ADMIN_KEY  — пароль для доступа к админке чата

Опционально:
  - DB_PATH    — путь к файлу базы SQLite (по умолчанию chat.db)
  - PORT       — порт для сервера (Render задаёт сам)
...
ADMIN_KEY — придумай любой сложный пароль, он нужен для доступа к админке чата.

Деплой на Render:
  1. Залей код в репозиторий (убедись, что в коде НЕТ токенов и ключей).
  2. В Render создай Web Service, подключи репозиторий.
  3. В разделе Environment Variables добавь BOT_TOKEN, CHAT_ID, ADMIN_KEY.
  4. Deploy — и всё.

После деплоя вставь полученный адрес бэкенда в index.html:
  LEAD_ENDPOINT  = 'https://твой-адрес.onrender.com/api/lead'
  CHAT_ENDPOINT  = 'https://твой-адрес.onrender.com/api/chat/send'
"""
...
# Проверяем, что всё нужное задано
if not BOT_TOKEN:
    raise RuntimeError("Переменная BOT_TOKEN не задана в окружении!")
if not CHAT_ID:
    raise RuntimeError("Переменная CHAT_ID не задана в окружении!")
if not ADMIN_KEY:
    raise RuntimeError("Переменная ADMIN_KEY не задана в окружении! Придумайте сложный пароль.")
...
if __name__ == "__main__":
    # Render передаёт порт через переменную PORT, для локального теста по умолчанию 5000
